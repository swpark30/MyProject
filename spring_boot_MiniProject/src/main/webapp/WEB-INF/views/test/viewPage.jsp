...
<html>
	<head>
		...
		<link
			...
			rel="stylesheet">
			<link href="<c:url value="/image/logo.png"/> " rel="shortcut icon" type="image/x-icon">
		<script
			src="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/js/bootstrap.bundle.min.js"></script>
		<script src="https://kit.fontawesome.com/51724f5bf4.js"></script>
		<script src="<c:url value="/js/test/view.js"/>" ></script>
		<script src="<c:url value="/js/test/writeComment.js"/>" ></script>
		<script src="<c:url value="/js/test/updateComment.js"/>" ></script>
		<link rel="stylesheet" type="text/css" href="<c:url value="/css/common.css"/>" >
		<link rel="stylesheet" type="text/css" href="<c:url value="/css/hashtag2.css"/>" >
		<link rel="stylesheet" type="text/css" href="<c:url value="/css/viewPage.css"/>" >
		...
		<link rel="stylesheet" type="text/css" href="<c:url value="/css/viewPage2.css"/>" >
		<title>게시글</title>
	</head>
	<body>
									...
									<div class="container" style="padding: 10px;">
										<div class="viewBtnBox row">
											<div class="viewBtn col-auto">
												<button id="btn" class="btn-gradient red" onclick="location.href='/test/deleteNote/${note.noteId }'">삭제</button>
												<button id="btn" class="btn-gradient blue" onclick="location.href='/test/updateNote/${note.noteId }'">수정</button>
											</div>
										</div>
									</div>
								</nav>
												...
												<div class="col-8">
													<table style="float: left;">
														<tr>
															...
															<td><a class="nickNameB" href="#">${note.userNickname }</a><span><i
																	class="fa-solid fa-2xs fa-bolt"></i></td>
														</tr>
														<tr>
															<td class="createdDateB"><fmt:formatDate pattern="yyyy-MM-dd hh:mm:ss" value="${note.noteCreatedDate }"/></td>
														</tr>
													</table>
												</div>
											...
											<div class="row viewSubWrap"
												...
												<div class="col-lg-12 col-12" style="padding: 5px;">
													...
													<span class="text-left"
														style="font-size: 1.1em; font-weight: bold; color: white;">#${note.noteId }</span>
														<a href="#" class="category1">Tips & 강좌</a> 
									...
									<li>
										<div class="container">
											...
											<c:forEach var="comment" items="${commentList}" varStatus="status">
												<div class="row viewSubWrap" id="commentBox${comment.commentId }">
													
													<input type="hidden" id="commentId${comment.commentId }" value="${comment.commentId }">
													<input type="hidden" id="userNickname${comment.commentId }" value="${comment.userNickname }">
													<input type="hidden" id="userImg${comment.commentId }" value="${comment.userImg }">
													<input type="hidden" id="date${comment.commentId }" value="${comment.commentCreatedDate }">
													
													<div class="row">
														<table style="float: left;">
															<tr>
																<td rowspan="2" width="40px"><img class="viewProfle"
																	src="${comment.userImg }"></td>
																<td><a class="nickNameB" href="#">${comment.userNickname }</a><span><i
																		class="fa-solid fa-2xs fa-bolt"></i> 
																		<c:if test="${sessionScope.sid == comment.userId }" >
																			<input type="button" onclick="updateForm(${comment.commentId })" value="수정"> 
																			<input type="button" onclick="deleteComment(${comment.commentId })"   value="삭제">
																		</c:if>
																			</td>
																		
															</tr>
															<tr>
																<td class="createdDateB"><fmt:formatDate pattern="MM-dd" value="${comment.commentCreatedDate }" /></td>
															</tr>
														</table>
													</div>
													<div class="row" style="padding: 10px">
														<p class="content-text" itemprop="articleBody" id="comment${comment.commentId }">${comment.comment }</p>
													</div>
												</div>
												...
												<div class="col-lg-9 col-9">
													<div class="row">
														<table style="float: left;">
															...
															<tr>
																<td class="createdDateB"><fmt:formatDate value="${now}" pattern="hh:mm" /></td>
															</tr>
														</table>
													</div>
													<div class="row" style="padding: 10px">
